...
from load_data import load_dataset, split_data,pltgraph,visualize_tsne,color_map_dict
...
pltgraph(gt, 1, 'TRUE', dataset_name,True)
...
for curr_seed in Seed_List:
    ...
    print('tran total time:',training_time)
    ...
    with torch.no_grad():
        ...
        output = net(net_input)
        if curr_seed==1:
            output=visualize_tsne(output,gt,color_map_dict,'SGCNIP',dataname=dataset_name)#*****************************************
        ...
        #pltgraph(gt,output,'SGCN',dataset_name)
        #print('------------------------graph-----------------------')
...
print("Average training time:", np.mean(Train_Time_ALL), '+-', np.std(Train_Time_ALL))
