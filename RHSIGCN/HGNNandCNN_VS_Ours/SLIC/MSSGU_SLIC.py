...
class SegmentMap(object):
    ...
    def getHierarchy(self):
        ...
        for i in range(layers - 1):
            ...
            l1 = np.reshape(segs[i], [-1])  # 上一层标签展平
            l2 = np.reshape(segs[i + 1], [-1])  # 下一层标签展平
            for x in range(h * w):
                if S[l1[x], l2[x]] != 1: S[l1[x], l2[x]] = 1  # 标记对应关系
            S_list.append(S)
        ...
        for l in range(len(superpixelLabels)):
            ...
            # 遍历图像，检测2x2窗口内的标签差异以确定邻接
            for i in range(h - 1):
                for j in range(w - 1):
                    sub = segments[i:i + 2, j:j + 2]  # 2x2窗口
                    sub_max = np.max(sub).astype(np.int32)
                    sub_min = np.min(sub).astype(np.int32)
...
if __name__ == '__main__':
    ...
    # 测试不同层次的超像素重建效果
    S1 = np.array(S_list[0])
    norm_col_S1 = S1 / (np.sum(S1, 0, keepdims=True))  # 列归一化（下一层到上一层的聚合）
    norm_row_S1 = S1  # 行归一化（上一层到下一层的分割）
    img = np.matmul(norm_col_S1.T, np.reshape(data, [-1, bands]))  # 聚合到更粗层
    img = np.matmul(norm_row_S1, img)  # 分割回原始层
