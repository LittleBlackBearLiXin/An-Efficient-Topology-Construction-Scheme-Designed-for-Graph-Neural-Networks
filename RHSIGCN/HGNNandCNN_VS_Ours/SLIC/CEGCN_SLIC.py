...
class SLIC(object):
    ...
    def get_Q_and_S_and_Segments(self):
        ...
        self.S = S  # 保存 S 矩阵,超像素内的所有像素点特征（波段）的平均值
        self.Q = Q  # 保存 Q 矩阵
    ...
    def get_A(self, sigma: float):
        '''
    根据 segments 判定超像素之间的邻接矩阵 A
    A[i, j] 表示第 i 个和第 j 个超像素是否邻接，以及它们之间的相似度（基于特征差异）
    :param sigma: 控制相似度计算中高斯核的尺度参数
    :return: 超像素邻接矩阵 A（大小为 超像素数 × 超像素数）
    '''
        # 初始化邻接矩阵 A，所有值设为 0，表示初始没有邻接关系
        ...
        for i in range(h - 2):
            for j in range(w - 2):
                ...
                # 如果这两个值不同，说明这 2x2 区域中至少存在两个不同的超像素 => 它们是邻接的
                if sub_max != sub_min:
                    idx1 = sub_max
                    idx2 = sub_min
...
class LDA_SLIC(object):
    ...
    def SLIC_Process(self,img,scale=25):
        ...
        A = myslic.get_A(sigma=10)  # 获取区域邻接矩阵或相似度矩阵
        return Q, S, A, Segments  # 返回结果
    ...
    def simple_superpixel(self,scale):
        ...
        Q, S, A, Seg = self.SLIC_Process(X, scale=scale)  # 对降维后的图像执行 SLIC 超像素分割
        return Q, S, A, Seg  # 返回超像素结果
    
    def simple_superpixel_no_LDA(self,scale):
        # 执行不包含 LDA 降维的超像素分割过程，直接对原始图像处理
        Q, S, A, Seg = self.SLIC_Process(self.data, scale=scale)  # 对原始图像执行 SLIC 超像素分割
        return Q, S, A, Seg  # 返回结果