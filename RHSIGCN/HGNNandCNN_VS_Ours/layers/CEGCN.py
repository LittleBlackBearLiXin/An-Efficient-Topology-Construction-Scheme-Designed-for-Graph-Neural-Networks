...
class GCNLayer(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, A: torch.Tensor,tranin=False):
        super(GCNLayer, self).__init__()
        self.A = A
        self.BN = nn.BatchNorm1d(input_dim)
        self.Activition = nn.LeakyReLU()
        self.sigma1 = torch.nn.Parameter(torch.tensor([0.1], requires_grad=False))
        # 第一层GCN
    ...
    def forward(self, H, model='normal'):#train=Flase,
        H = self.BN(H)
...
class OURSCEGCN(nn.Module):
    def __init__(self, height: int, width: int, changel: int, class_count: int, Q: torch.Tensor, A: torch.Tensor,AX,
        ...
        for i in range(MPNNlayers_count):
            ...
            if i < MPNNlayers_count - 1:
                self.MPNN_Branch.add_module('GCN_Branch' + str(i), GNN(input=128,output= 128,droupout=0,A=self.AX))
            else:
                self.MPNN_Branch.add_module('GCN_Branch' + str(i), GNN(input=128,output= 64,droupout=0,A=self.AX))
    ...
    def forward(self, x: torch.Tensor):
        ...
        GCN_result = torch.matmul(self.Q, H)+MPNNruslt  # 这里self.norm_row_Q == self.Q
